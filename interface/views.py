...
def inference(image, background_enhance, face_upsample, upscale, codeformer_fidelity):
    ...
    try: # global try
        ...
        # save restored img
        save_path = f'media/images/out.png'
...
def index(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            fidility = form.cleaned_data['fidility']
            upsample = form.cleaned_data['upsample']
            bgEnhance = form.cleaned_data['bgEnhance']

            inference(path,bgEnhance,upsample,2,fidility)

            result = True

            return render(request, 'index.html', {'form': form, 'img_obj': img, 'result': result})  
    else:
        form = UploadForm()
